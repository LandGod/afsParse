try:
    ...
    with open(filename, newline='') as csvfile:
        ...
        # If we encounter an error, inform the user so that they can see what the heck is going on
        except UnicodeDecodeError as e:
            print("The program encountered a decoding error while trying to read the file and was unable to continue.")
            print("The file you are trying to process may be corrupted.")

        except Exception as e:
            print("The program encountered an unknown error while reading the file and is unable to continue.")
            print("*****************************************************************************\n")
            print(repr(e))
except TypeError as e:
    if e.args[0] == 'Incorrect file type! Filetype must be: .csv':
        print("Error: Invalid file type!")
        print("The supplied file had the extention '{}', but the program can only extract data from files with the extention '.csv'.".format(e.args[1]))
    else:
        raise
        
except Exception as e:
    print("The program crashed due to an unknown error:")
    print(repr(e))
except:
    print("The program crashed due to an unknown non-Python error:")
